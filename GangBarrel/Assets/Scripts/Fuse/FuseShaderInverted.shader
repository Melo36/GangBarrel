Shader "Custom/FuseBurningOverlayInverted"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _BurnProgress ("Burn Progress", Range(0,1)) = 0
        _UnburntColor ("Unburnt Color", Color) = (0.4,0.25,0.25,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float invertedY = 1.0 - i.uv.y;
                
                float burnEdge = _BurnProgress;
                float burningZone = smoothstep(burnEdge - _BurnWidth, burnEdge, invertedY) * 
                                  (1 - smoothstep(burnEdge, burnEdge + _BurnWidth, invertedY));
                
                float unburnt = step(burnEdge + _BurnWidth, invertedY);
                float burnt = step(invertedY, burnEdge - _BurnWidth);
